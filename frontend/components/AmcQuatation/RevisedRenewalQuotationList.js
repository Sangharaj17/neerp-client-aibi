...
export default function RevisedRenewalQuotationList({ quotationId }) {
  ...
  const fetchRevisedQuotations = async () => {
    try {
      ...
      const response = await axiosInstance.get(
        `/api/amc/quotation/renewal/revise/by-quotation/${quotationId}`
      );
  ...
  const handleRevise = (id) => {
    ...
    window.location.href = `/dashboard/quotations/amc-renewal-quatation-list/amc-renewal-revised-quatation-list/${quotationId}/revision/${id}`;
  };
  ...
  return (
    <div className="min-h-screen">
      <Head>
        <title>Revised Renewal Quotation List</title>
    ...
    <h1 className="text-2xl font-bold text-gray-800">
      Revised Renewal Quotation List
    </h1>
