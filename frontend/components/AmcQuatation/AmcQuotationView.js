...
export default function AmcQuotationView({ params }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        let url = `/api/amc/quotation/initial/${quotationId}`;

        if(revision == true){
            url = `/api/amc/quotation/initial/revised/getQuotationViewDetails/${quotationId}`;
        }
                ...
                {data.combinedQuotations?.length > 0 ? (
                  data.combinedQuotations.map((cq, index) => (
                    ...
                    <td className="border border-gray-300 p-2">{cq.typeOfElevators || "Manual"}</td>
                      <td className="border border-gray-300 p-2">{cq.amountOrdinary || "10000.00"}</td>
                      <td className="border border-gray-300 p-2">{cq.gstOrdinary || "0.00"}</td>
                      <td className="border border-gray-300 p-2">{cq.totalAmountOrdinary || "30000.00"}</td>
