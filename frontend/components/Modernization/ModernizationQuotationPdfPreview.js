...
/* ===========================
   Main Component
   =========================== */
export default function ModernizationQuotationPdfPreview({ 
  modernizationId,
    ...
    const fetchData = async () => {
      try {
        const response = await axiosInstance.get(
          `/api/modernization/getModernizatationQuotationPdfData/${modernizationId || 1}`
        );
        setApiData(response.data || {});
      } catch (err) {
        console.error("Error fetching modernization data:", err);
        setApiData({});
      } finally {
        setIsLoading(false);
      }
  ...
  if (isLoading) {
    return <div style={{ padding: 12 }}>Loading Modernization Quotation Data...</div>;
  }

  // Get site name from API data
  const pdfFileName = `${apiData?.sitename || "Site"}_ModernizationQuotation.pdf`;
