...
const ModernizationInvoicePrint = ({ invoiceId = 2, onBackToList }) => {
  ...
  useEffect(() => {
    const fetchInvoiceData = async () => {
      try {
        setLoading(true);
        const response = await axiosInstance.get(`/api/modernization/getModernizationInvoiceData/${invoiceId}`);
        setInvoiceData(response.data);
        setError(null);
      } catch (err) {
        console.error('Error fetching modernization invoice data:', err);
        setError('Failed to fetch modernization invoice data.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading)
    return <div className="w-full mx-auto p-4 text-center text-gray-700">Loading Invoice...</div>;
  if (error)
    return <div className="w-full mx-auto p-4 text-center text-red-600 font-bold">{error}</div>;
  if (!invoiceData)
    return <div className="w-full mx-auto p-4 text-center text-gray-700">No invoice data available.</div>;
...
export default ModernizationInvoicePrint;
