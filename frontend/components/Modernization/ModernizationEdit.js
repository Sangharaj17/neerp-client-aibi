...
const ModernizationEdit = ({ id, onSave }) => {
  ...
  const [form, setForm] = useState({
    ...
    amount: 0,
    amountWithGst: 0,
    // isFinal: false, // REMOVED
    // quotationFinalDate: '', // REMOVED
    gstApplicable: 'yes',
    gstPercentage: 18,
    subtotal: 0,
    gstAmount: 0,
    ...
    const fetchData = async () => {
      ...
      try {
        const { data } = await axiosInstance.get(
          `/api/modernization/getModernizationQuotationById/${id}`
        );
        ...
        if (combinedEnquiryId) {
          const preRes = await axiosInstance.get(
            `/api/modernization/pre-data/${combinedEnquiryId}`
          );
      ...
      <header className="flex justify-between items-center pb-6 border-b mb-6">
        <div>
          <h2 className="text-3xl font-bold text-gray-900">
            Edit Quotation: {form.quotationNo || '-'}
          </h2>
