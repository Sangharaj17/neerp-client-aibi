...
const RenewalJobDetailPage = ({ jobId }) => {
    ...
    const fetchJobDetail = async () => {
      try {
        ...
        // );
           ...
           const finalUrl = `/api/jobs/renewal/amc-renewal-job-activities/job-detail/${jobId}`;
  ...
  if (!jobDetails) {
    return <div className="p-4 text-center text-red-500">Job details not found.</div>;
  }

  const handleBack = (label) =>{
    setLoadingBtn(label);
    router.push(`/dashboard/jobs/amc_job_list/true`);
  }

  const handleAddActivity = (label) =>{
    setLoadingBtn(label);
    router.push(`/dashboard/jobs/add-renewal-job-activity/${jobId}`);
  }

  const exportJobActivityToExcel = (label) => {
      setLoadingBtn(label);
    exportToExcel();

  };

  const exportActivityDetailsToPDF = (label) => {
      setLoadingBtn(label);
    exportToPDF();
  };
    ...
    const worksheet = XLSX.utils.json_to_sheet(jobActivities.map(act => ({
      ID: act.id,
      Date: act.date,
      ...
      <div className="flex flex-wrap justify-center gap-2">
        {[
          ...
          >
            {label}
            {loadingBtn === label ? 
<Loader2 className="w-4 h-4 animate-spin text-orange-500" /> : null}
      ...
      <div className="border border-gray-300 rounded-lg overflow-hidden">
        <div className="flex justify-between items-center bg-gray-100 px-3 py-2">
          <h2 className="text-sm font-semibold text-gray-700">Job Information</h2>
  ...
  >
    View Employee Details
              ...
              <div key={i} className="flex border-b border-gray-300">
                <div className="w-1/2 bg-gray-100 px-3 py-2 text-sm font-medium">{label}</div>
      ...
      <div className="flex justify-center">
        <div className="w-full max-w-6xl mx-auto">
          <h2 className="text-lg font-semibold mb-2 text-center">Job Activity Details</h2>
                ...
                {jobActivities.length > 0 ? (
                  jobActivities.map((activity, idx) => (
                    <tr key={activity.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2 border border-gray-300 text-sm">{activity.date}</td>
                      <td className="px-4 py-2 border border-gray-300 text-sm">{activity.activityBy}</td>
                      <td className="px-4 py-2 border border-gray-300 text-sm">{activity.activityType}</td>
                      <td className="px-4 py-2 border border-gray-300 text-sm">{activity.serviceType}</td>
                      <td className="px-4 py-2 border border-gray-300 text-sm">{activity.description}</td>
                      <td className="px-4 py-2 border border-gray-300 text-sm">{activity.remark || "-"}</td>
                    <td className="px-4 py-2 border border-gray-300 text-sm">{activity.liftName || "-"}</td>
      ...
      {/* Employee Modal using ActionModal */}
<ActionModal isOpen={isEmployeeModalOpen} onCancel={() => setIsEmployeeModalOpen(false)}>
  <h2 className="text-2xl font-bold mb-6 text-gray-800 text-center">Employee Details</h2>
        ...
        >
          ...
          <div className="mt-14 grid grid-cols-1 gap-4">
            <div className="flex flex-col">
              ...
              <span className="text-gray-800 font-medium">{emp.name || "N/A"}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-gray-500 text-sm mb-1">Role</span>
              <span className="text-gray-800 font-medium">{emp.role || "N/A"}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-gray-500 text-sm mb-1">Address</span>
              <span className="text-gray-800 font-medium">{emp.address || "N/A"}</span>
            </div>
          </div>
        </div>
      ))
    ) : (
      <p className="col-span-full text-center text-gray-500">No employee data found.</p>
    )}
  </div>
</ActionModal>
...
{/* QR Code Generation Modal */}
      ...
      <ActionModal isOpen={isLiftModalOpen} onCancel={() => setIsLiftModalOpen(false)}>
        <h2 className="text-2xl font-bold mb-6 text-gray-800 text-center">Lift Details</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {liftDatas.length > 0 ? (
            liftDatas.map((lift, index) => (
              ...
              >
                ...
                <div className="mt-14 grid grid-cols-2 gap-4">
                  <div className="flex flex-col">
                    <span className="text-gray-500 text-sm mb-1">Lift Name</span>
                    <span className="text-gray-800 font-medium">{lift.liftName || "N/A"}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-gray-500 text-sm mb-1">Capacity</span>
                    <span className="text-gray-800 font-medium">{lift.capacityValue || "N/A"}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-gray-500 text-sm mb-1">Type</span>
                    <span className="text-gray-800 font-medium">{lift.typeOfElevators || "N/A"}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-gray-500 text-sm mb-1">Floors</span>
                    <span className="text-gray-800 font-medium">{lift.noOfFloors || "N/A"}</span>
                  </div>
                </div>
              </div>
            ))
          ) : (
            <p className="col-span-full text-center text-gray-500">No lift data found.</p>
          )}
        </div>
      </ActionModal>
...
export default RenewalJobDetailPage;
