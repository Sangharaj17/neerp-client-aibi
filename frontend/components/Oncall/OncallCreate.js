...
const OncallCreate = ({ leadId, combinedEnquiryId, customer, site }) => {
  ...
  // --- Calculate Totals Logic (No Change) ---
  ...
  // --- Recalculate Totals whenever details or GST percentage/applicability change (No Change) ---
  useEffect(() => {
  ...
  // --- Fetch OnCall pre-data (MODIFIED API ENDPOINT) ---
  useEffect(() => {
    if (!combinedEnquiryId) return;

    const fetchPreData = async () => {
      try {
        const res = await axiosInstance.get(`/api/oncall/preData/${combinedEnquiryId}`);
        const data = res.data;
        ...
        console.error('Failed to load OnCall pre-data:', err);
        toast.error('Failed to load OnCall pre-data');
      }
    };
  ...
  const handleChange = (e) => {
    ...
    if (name === 'gstPercentage' && type === 'number') {
      finalValue = parseFloat(value) || 0;
    }
    // HSN/SAC is typically a string, even if it looks like a number, so no forced float conversion
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      // Updated property name from 'modernization' to 'oncall'
      oncallDto: {
        ...form,
        leadId,
        combinedEnquiryId,
        ...
        amount: form.subtotal, // amount field is subtotal before tax
    ...
    try {
      // Using the OnCall CREATE API: /api/oncall (POST)
      await axiosInstance.post('/api/oncall', payload);
      toast.success('OnCall quotation created successfully!');
      router.push('/dashboard/quotations/oncall-quotation-list'); // Assuming the OnCall list route is OncallList
    } catch (error) {
      console.error('Submission Error:', error);
      const errorMessage = error.response?.data?.message || 'Failed to save OnCall quotation';
      toast.error(errorMessage);
    }
  };

  // Helper for input styling for reusability (No Change)
  ...
  return (
    <div className="max-w-7xl mx-auto bg-white shadow-2xl rounded-xl p-8 my-8">
      <header className="flex justify-between items-center pb-6 border-b border-gray-200 mb-6">
        <div>
          <h2 className="text-3xl font-extrabold text-gray-900">Create OnCall Quotation</h2>
      ...
      {/* Lift Info (Retained as OnCall likely applies to a lift) */}
      {liftDatas.length > 0 && (
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <section>
          ...
          {/* Corrected grid to align HSN/SAC, GST, and Final fields properly */}
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mt-4 items-end">
            ...
            <div className="relative col-span-1">
              ...
              <div className="flex items-center relative">
                <FaCode className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 h-4 w-4" />
                <input
                  type="text"
                  name="hsnSacCode"
            ...
            <div className="relative col-span-1">
              ...
              <input
                ...
                onChange={handleChange}
                className={form.gstApplicable === 'no' ? readOnlyStyle : inputStyle}
        ...
        {/* Details Table (No Change in structure) */}
        ...
        {/* Totals Summary (No Change in structure) */}
