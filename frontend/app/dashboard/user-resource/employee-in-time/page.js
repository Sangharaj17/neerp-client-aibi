...
const API_EMPLOYEE_IN_TIME = "/api/employee-in-time";
const API_EMPLOYEES = "/api/employees";

export default function EmployeeInTimePage() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (formData.inTimeHours === "" || formData.inTimeMinutes === "") {
      toast.error("Please select in time hours and minutes.");
      return;
    }
    if (!formData.date) {
    ...
    if (selectedDate < today) {
      toast.error("Cannot mark in-time for past dates. Please select today's date or a future date.");
      return;
    }
    ...
    try {
      const payload = {
        ...
        inTimeHours: parseInt(formData.inTimeHours),
        inTimeMinutes: parseInt(formData.inTimeMinutes),
        date: formData.date,
        assignedWork: formData.assignedWork || null,
      };

      await axiosInstance.post(API_EMPLOYEE_IN_TIME, payload);
      toast.success("Employee in time added successfully.");
      ...
      console.error("Failed to add employee in time", error);
      ...
      if (error.response) {
        ...
        let message = "Failed to add employee in time.";
        ...
        toast.error(`Failed to add employee in time: ${error.message || 'Unknown error'}`);
      }
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-6">
      <header className="space-y-2">
        <h1 className="text-2xl font-semibold text-gray-900">Add Your In time</h1>
        <p className="text-sm text-gray-600">
          Record employee in-time for attendance tracking.
        </p>
      </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Enter assigned work details..."
              disabled={saving}
            />
          </div>
