...
export default function CopType() {
  ...
  const [data, setData] = useState([
    {
      id: 1,
      operatorType: 'Manual',
      copType: 'C.O.P WITH INDIAN SWITCH WITH SURFACE MOUNTED S.S.HAIRLINE FOR MANUAL LIFT',
      price: '8500',
    },
    {
      id: 2,
    ...
    <div className="space-y-8 w-full p-6 min-h-screen">
      ...
      <div>
        <h2 className="text-lg font-medium text-gray-800 flex items-center gap-2">
          ...
          Control Panel (COP) Type
        </h2>
        <p className="text-sm text-gray-500">Manage and list control panel types.</p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* COP Type */}
          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
            <label className="sm:w-1/4 font-medium text-gray-700">COP Type*</label>
            <input
              required
              type="text"
              value={form.copType}
              onChange={(e) => setForm({ ...form, copType: e.target.value })}
              placeholder="Enter COP Type"
              className="w-full sm:w-3/4 border border-gray-300 rounded p-2 text-sm"
            />
          </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
        <h3 className="text-sm font-medium text-gray-700 mb-4">COP Type List</h3>
          ...
          <table className="w-full text-sm border border-gray-200 rounded-md overflow-hidden">
            <thead className="bg-gray-100 text-gray-700">
              <tr>
                ...
                <th className="px-4 py-2 border border-gray-200 text-left">COP Type</th>
              ...
              {data.length > 0 ? (
                data.map((item, idx) => (
                  <tr key={item.id} className="border-t border-gray-200 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 border border-gray-200">{item.copType}</td>
