...
export default function ControlPanelPage() {
  const API_URL = API_ENDPOINTS.CONTROL_PANEL; //"/api/control-panel-types";
  ...
  const handleSubmit = async (e) => {
    ...
    //console.log(controlPanelTypes, "----------", newItem, "---newItem-------");

    // Duplicate check
    const duplicate = controlPanelTypes.some((s) => {
      const existingValueId =
        s.capacityTypeId === 1 ? s.personCapacityId : s.weightId;
      const newValueId =
        selectedCapacity.id === 1 ? newItem.personCapacityId : newItem.weightId;

      return (
        s.operatorTypeId === newItem.operatorTypeId &&
        s.machineTypeId === newItem.machineTypeId &&
        s.capacityTypeId === newItem.capacityTypeId &&
        existingValueId === newValueId &&
        s.id !== editId // exclude self on edit
      );
    });
