...
export default function GovernorRopePage() {
  ...
  const [data, setData] = useState([
    { id: 1, ropeType: 'OVERSPEED GOVERNER SAFETY ROPE 8 MM G+1', floor: 'G+1', quantity: '20 mts', price: 1300 },
    { id: 2, ropeType: 'OVERSPEED GOVERNER SAFETY ROPE 8 MM G+2', floor: 'G+2', quantity: '28 mts', price: 1820 },
    { id: 3, ropeType: 'OVERSPEED GOVERNER SAFETY ROPE 8 MM G+3', floor: 'G+3', quantity: '32 mts', price: 2080 },
  ]);
  ...
  const handleSubmit = (e) => {
    ...
    const newItem = {
      id: editId || Date.now(),
      ropeType: form.ropeType.toUpperCase(),
      floor: form.floor,
      quantity: `${form.quantity} mts`,
      price: form.price,
    };
    ...
    setForm({ ropeType: '', floor: '', quantity: '', price: '' });
  };
  ...
  const handleEdit = (item) => {
    setForm({
      ...
      floor: item.floor,
      quantity: item.quantity.replace(' mts', ''),
      price: item.price,
    });
    setEditId(item.id);
  };
  ...
  return (
    <div className="space-y-8 w-full p-6 min-h-screen">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500">Add overspeed governor safety rope data per floor.</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
            <label className="text-sm font-medium text-gray-700 min-w-[180px] text-center sm:text-right">
              Governor Rope Type<span className="text-red-500">*</span>
            </label>
            <input
              required
              type="text"
              value={form.ropeType}
              onChange={(e) => setForm({ ...form, ropeType: e.target.value })}
              className="border border-gray-300 rounded-md px-4 py-2 w-full sm:w-72 text-sm"
              placeholder="Rope type"
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
            <label className="text-sm font-medium text-gray-700 min-w-[180px] text-center sm:text-right">
              Quantity (mts)<span className="text-red-500">*</span>
            </label>
            <input
              required
              type="number"
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
        <h3 className="text-sm font-medium text-gray-700 mb-4">Governor Rope List</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-sm border border-gray-200 rounded-md overflow-hidden">
            <thead className="bg-gray-100 text-gray-700">
              <tr>
                <th className="px-4 py-2 border border-gray-200 text-left">Sr.No.</th>
                <th className="px-4 py-2 border border-gray-200 text-left">Rope Type</th>
              ...
              {data.length > 0 ? (
                data.map((item, idx) => (
                  <tr key={item.id} className="border-t border-gray-200 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 border border-gray-200">{item.ropeType}</td>
