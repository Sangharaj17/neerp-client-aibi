...
export default function LopTypeAndSubType() {
  ...
  const handleTypeSubmit = async (e) => {
    ...
    if (isDuplicate) {
      toast.error("LOP Type already exists with selected Operator.");
      return;
    }
    ...
    confirmDeleteWithToast(selected.lopType, async () => {
      try {
        ...
        toast.error(
          err.response?.data?.message ||
            err.message ||
            "Error deleting Landing Door Type"
        );
      }
    });
  };
  ...
  const handleSubSubmit = async (e) => {
    ...
    const isDuplicate = lopSubTypes.some(
      (t) =>
        ...
        t.lopTypeId == subForm.lopType &&
        t.subTypeId !== editSubId
    );
    if (isDuplicate) {
      toast.error(
        "LOP SubType already exists with selected Operator and Door Type."
      );
      return;
    }
